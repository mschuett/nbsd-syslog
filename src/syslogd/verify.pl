...
my ( @a,       @b );
my ( $host, $ver,  $rsid, $sg,  $spri, $tbpl, $index, $flen,
     $frag, $sign, $gbc,  $fmn, $cnt,  $hb,   $text );
...
for my $i ( 0 .. $#CBlist ) {
    ( $host, $ver, $rsid, $sg, $spri, $tbpl, $index, $flen, $frag, $text ) =
      @{ $CBlist[$i] };
    ...
    if ( $flen != length($frag) ) {
        print STDERR
          "Warning: ignore CB with invalid lenght field ($flen != length($frag))\n"
          unless $quiet;
        next;
    }
...
for my $i ( 0 .. $#SBlist ) {
    ...
    if    ( $ver eq "0111" ) { $msglist = \%msglist_sha1; }
        ...
        print STDERR "Error: found SB with invalid version field: $ver\n"
          unless $quiet;
    }
    ...
    if ( $hbslen != $cnt ) {
        print STDERR
          "Warning: found wrong number of hashes in SB: $hbslen != $cnt\n"
          unless $quiet;
    }
    my $i = 0;
    while ( $i < $hbslen ) {
...
sub read_DER_DSA {
    ...
    my $pubkey_der =
      ...
      . $b64oneline . "\n"
      . "-----END PUBLIC KEY-----\n";
    my $dsakey = eval { Crypt::OpenSSL::DSA->read_pub_key_str($pubkey_der) };
    if ($@) {
...
sub read_PKIX {
    ...
    my $cert =
      Crypt::OpenSSL::X509->new_from_string( $der,
    ...
    if ( $pubkey_der =~ /^-----BEGIN RSA PUBLIC KEY-----/ ) {
        ...
        print STDERR
          "Warning: got PKIX RSA key, but cannot use it to verify signatures\n"
          unless $quiet;
